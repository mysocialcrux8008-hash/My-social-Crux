    ...
    <>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
            {caseStudies.map((cs, i) => (
              <motion.div key={cs.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                style={{ borderRadius: "20px", overflow: "hidden", border: "1px solid rgba(255,255,255,0.08)", background: "#0B0F19" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr" }} className="cs-grid">
                  {/* Left panel */}
                  <div style={{ padding: "32px 28px", background: `linear-gradient(135deg, ${cs.color}15, transparent)`, borderRight: "1px solid rgba(255,255,255,0.06)" }} className="cs-left">
                    <div style={{ display: "inline-block", padding: "5px 12px", borderRadius: "999px", background: cs.color + "20", color: cs.color, fontSize: "11px", fontWeight: 600, marginBottom: "14px" }}>{cs.industry}</div>
                    <h2 style={{ fontFamily: "'Sora', sans-serif", fontSize: "clamp(18px,2.5vw,24px)", fontWeight: 800, color: "#FFFFFF", marginBottom: "10px" }}>{cs.client}</h2>
                    <p style={{ color: "#9CA3AF", fontSize: "13px", lineHeight: 1.6, marginBottom: "10px" }}>{cs.challenge}</p>
                    <p style={{ color: "#D1D5DB", fontSize: "13px", lineHeight: 1.6, marginBottom: "16px" }}>{cs.result}</p>
                  ...
                  {/* Right metrics */}
                  <div style={{ padding: "32px 24px" }} className="cs-right">
                    <p style={{ color: "#6B7280", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: 600, marginBottom: "20px" }}>Key Results</p>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "12px" }} className="cs-metrics">
                      {cs.metrics.map((m) => (
                        <div key={m.label} style={{ borderRadius: "14px", padding: "16px 12px", textAlign: "center", background: cs.color + "10", border: `1px solid ${cs.color}25` }}>
                          ...
                          <div style={{ fontFamily: "'Sora', sans-serif", fontSize: "clamp(16px,2vw,24px)", fontWeight: 900, color: "#FFFFFF", marginBottom: "4px" }}>{m.after}</div>
                          <div style={{ color: "#9CA3AF", fontSize: "11px", marginBottom: "3px" }}>{m.label}</div>
                          <div style={{ color: "#4B5563", fontSize: "10px" }}>from {m.before}</div>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      <style>{`
        @media(max-width:768px){
          ...
          .cs-left{border-right:none!important; border-bottom:1px solid rgba(255,255,255,0.06)!important; padding:24px 20px!important;}
          .cs-right{padding:24px 20px!important;}
